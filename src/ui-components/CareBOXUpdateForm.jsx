...
export default function CareBOXUpdateForm(props) {
  const {
    id: idProp,
    careBOX: careBOXModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = careBOXRecord
      ? { ...initialValues, ...careBOXRecord }
      : initialValues;
    setDeviceId(cleanValues.deviceId);
    setTimeStamp(cleanValues.timeStamp);
    setAlarmeO2(cleanValues.alarmeO2);
    setAlarmeCACO2(cleanValues.alarmeCACO2);
    setTemperatura(cleanValues.temperatura);
    setUmidade(cleanValues.umidade);
    setConcentracaoCO2(cleanValues.concentracaoCO2);
    setParticulados1um(cleanValues.particulados1um);
    setParticulados25um(cleanValues.particulados25um);
    setParticulados10um(cleanValues.particulados10um);
    setCargaO2(cleanValues.cargaO2);
    setErrors({});
  };
  const [careBOXRecord, setCareBOXRecord] = React.useState(careBOXModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(CareBOX, idProp)
        : careBOXModelProp;
      setCareBOXRecord(record);
    };
    queryData();
  }, [idProp, careBOXModelProp]);
  React.useEffect(resetStateValues, [careBOXRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            CareBOX.copyOf(careBOXRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CareBOXUpdateForm")}
      {...rest}
    >
      <TextField
        label="Device id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || careBOXModelProp)}
